...
class GenerateModelCommand extends Command
{
    ...
    private function generateClassType($file, $class, $properties, $types_reference, $types_description, $is_enum, $is_interface, $create_getter, $create_setter, $add_constructor, $filesystem, $io)
    {
        if (array_key_exists('extend', $properties)) {
            $class->setExtends($properties['extend']);
            $class->getNamespace()->addUse($properties['extend']);
        }
        if (array_key_exists('implements', $properties)) {
            $class->setImplements($properties['implements']);
            foreach ($properties['implements'] as $implement_use) {
                $class->getNamespace()->addUse($implement_use);
            }
        }
        if (array_key_exists('fields', $properties)) {
            if ($add_constructor) {
                //costruttore
                $mc = $class->addMethod('__construct');
                $mc->setStatic(false);
            ...
            foreach ($properties['fields'] as $name => $field_properties) {
                ...
                if (array_key_exists('class', $field_properties)) {
                    ...
                    if (strpos($field_class_name, '\\') !== 0) {
                        if (array_key_exists($field_class_name, $types_reference)) {
                            ...
                            $class->getNamespace()->addUse($field_namespace);
                            if ($add_constructor) {
                                $mc->addParameter($name)->setTypeHint($field_class_full);
                            }
                        } else {
                            $this->errors[] = ' Missing class '.$field_class_name.' on '.$class->getName();
                        }
                    } else {
                        if ($add_constructor && !$is_autoinizialize) {
                ...
                if (!$is_autoinizialize) {
                    ...
                    if (null !=  $default_value) {
                        ...
                        $this->logger->error('autoinizialize for '.$field_class_full.' not defined on element '.$class->getName());
                        $this->errors[] = 'autoinizialize for '.$field_class_full.' not defined on element '.$class->getName();
                    }
                }
                ...
                if ($is_interface) {
                    if ($create_getter) {
                        ...
                        $m = $class->addMethod('get'.ucfirst($name));
                    ...
                    if ($create_setter) {
                        /** $m @var \Nette\PhpGenerator\Method */
                        $m = $class->addMethod('set'.ucfirst($name));
                        $m->setStatic($is_static);
        ...
        $outFile = str_replace('\\', '/', $class->getNamespace()->getName().'\\'.$class->getName()).'.php';
        if ($filesystem->has($outFile)) {
            $filesystem->put($outFile, (string)$file);
        } else {
            $filesystem->write($outFile, (string)$file);
        }
